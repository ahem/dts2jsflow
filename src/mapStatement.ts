...
function mapTypeAliasDeclaration(node: ts.TypeAliasDeclaration): flow.Statement {
...
function mapVariableStatement(node: ts.VariableStatement): flow.Statement[] {
...
function mapFunctionDeclaration(node: ts.FunctionDeclaration): flow.Statement {
    if (!node.name) {
        throw new Error('not implemented');
    }
    if (!node.type) {
    ...
    id.typeAnnotation = flow.typeAnnotation(
        mapType(ts.createFunctionTypeNode(node.typeParameters, node.parameters, node.type)),
    );
...
function mapInterfaceDeclaration(node: ts.InterfaceDeclaration): flow.DeclareInterface {
    const id = flow.identifier(node.name.text);
    const properties = node.members.map(member => {
        if (ts.isPropertySignature(member)) {
            return mapPropertySignatureOrDeclaration(member);
        }
...
function mapClassDeclaration(node: ts.ClassDeclaration): flow.DeclareClass {
    if (!node.name) {
    ...
    const properties = node.members
        ...
        .map(member => {
            ...
            if (ts.isConstructorDeclaration(member)) {
                ...
                return flow.objectTypeProperty(flow.identifier('constructor'), mapFunction(member));
            }
            if (ts.isMethodDeclaration(member)) {
                return flow.objectTypeProperty(mapPropertyName(member.name), mapFunction(member));
            }
            throw new Error(
...
export function mapStatement(
    ...
    if (ts.isTypeAliasDeclaration(node)) {
        return mapTypeAliasDeclaration(node);
    } else if (ts.isVariableStatement(node)) {
        return mapVariableStatement(node);
        ...
        return mapInterfaceDeclaration(node);
